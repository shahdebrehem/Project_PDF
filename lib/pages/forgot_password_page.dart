...
class _ForgotPasswordPageState extends State<ForgotPasswordPage>
  ...
  Future<void> _sendResetLink() async {
    if (_emailController.text.trim().isEmpty) {
      setState(() => _errorMessage = isArabic ? 'البريد الإلكتروني مطلوب' : 'Email is required');
      return;
    }

    if (!_emailController.text.contains('@') || !_emailController.text.contains('.')) {
      setState(() => _errorMessage = isArabic ? 'الرجاء إدخال بريد إلكتروني صحيح' : 'Please enter a valid email');
      return;
    }
    ...
    try {
      ...
      if (response['success'] == true) {
        ...
        setState(() {
          _errorMessage = response['message'] ?? (isArabic ? 'فشل إرسال رابط إعادة التعيين' : 'Failed to send reset link');
        });
      }
    } catch (e) {
      if (mounted) {
  ...
  /// Input Container with elevation and shadow
